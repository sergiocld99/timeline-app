...
const CrossesPage = () => {
  ...
  const updateSet = (checked: CheckedState, item: Cross, setFn: Dispatch<SetStateAction<Cross[]>>) => {
    ...
    else { setFn(prev => prev.filter(c => c._id !== item._id)) }
  }
  ...
  const onAddCrosses = async (travelId: string) => {
    ...
    TravelService.update(travelId, { crosses })
      .then(t => { toast.success(`After addition, travel has ${t.crosses.length} crosses, wanted ${crosses.length}`) })
      .catch(err => { toast.error(`Failed to add crosses: ${err}`) })
  }
  ...
  const onRemoveCrosses = async (travelId: string) => {
    ...
    TravelService.update(travelId, { crosses })
      .then(t => { toast.success(`After deletion, travel has ${t.crosses.length} crosses, wanted ${crosses.length}`) })
      .catch(err => { toast.error(`Failed to remove crosses: ${err}`) })
