...
- **Never commit UI changes without explicit user confirmation that the rendered UI is correct.** After making a UI change, rebuild/restart the stack, tell the user how to check it, and wait for them to verify and say it looks right before committing — don't commit on lint/build/typecheck alone. (For non-UI changes, the Docker rebuild above remains the final gate.)
- **Every hook change ships with unit tests.** Whenever you add or modify a hook (anything under `frontend-v2/src/hooks/` or a custom hook anywhere in the frontend), add or extend the matching vitest spec under `frontend-v2/tests/` — hook logic is deterministic and unit-testable, so a behavioral hook change without a test is incomplete, and `npm test` must pass before finishing the task.
