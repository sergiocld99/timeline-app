...
- **`Location.name` uniqueness**: Node's `LocationSchema` indexes `{ name, latitude }` unique, not `zipcode` — multiple locations can share a zipcode. `PlacesVisitedDTO.data[zipcode]` keeps "last location wins" (iteration order over `context.locations`), matching Node's `Map.set` overwrite behavior exactly.
